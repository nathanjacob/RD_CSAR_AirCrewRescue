...
if (!_cache) then {
	if ismultiplayer then {
			...
			{_actCond="{vehicle _x in thisList && isplayer _x && ((getPosATL _x) select 2) < 30} count playableunits > 0";
							}else 
							{_actCond="{vehicle _x in thisList && isplayer _x} count playableunits > 0";
		};}else{
			if (_heightLimit) then 
						{_actCond="{vehicle _x in thisList && isplayer _x && ((getPosATL _x) select 2) < 30} count allUnits > 0";
								}else
